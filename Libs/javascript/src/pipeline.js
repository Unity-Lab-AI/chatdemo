...
export class ImageStep {
  constructor({ prompt, outKey, params = {} }) { this.prompt = prompt; this.outKey = outKey; this.params = params; }
  async run({ client, context }) { const b = await imageGen(this.prompt, this.params, client); context.set(this.outKey, { base64: Buffer.from(b).toString('base64') }); }
}

export class TtsStep {
  constructor({ text, outKey, params = {} }) { this.text = text; this.outKey = outKey; this.params = params; }
  async run({ client, context }) { const b = await ttsGen(this.text, this.params, client); context.set(this.outKey, { mp3_base64: Buffer.from(b).toString('base64') }); }
}
